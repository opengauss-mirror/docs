...
使用Docker实现openGauss容器化部署，简化DevOps用户的安装、配置和环境设置，参考 [容器镜像安装](../installation_guide/installing_the_container_image.md)。
...
`8.0.0` 版本引入 BM25 索引空间优化能力。通过可变大小块存储机制，在小文档或小倒排片段场景下可降低空间浪费，提升索引空间利用率。
