...
- 当在异常情况或表被损坏时需要使用[账本数据库的函数](../sql_reference/ledger_database_functions.md)章节中的ledger\_gchain\_repair\(text, text\)或ledger\_hist\_repair\(text, text\)接口对全局区块表或用户历史表进行修复，修复后调用全局区块表或用户历史表校验接口结果为true。
