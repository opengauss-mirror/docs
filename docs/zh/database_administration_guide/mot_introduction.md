...
MOT与基于磁盘的普通（堆）表并排创建。MOT的有效设计实现了几乎完全的SQL覆盖，并且支持完整的数据库功能集，如存储过程和自定义函数（限制参见[MOT SQL覆盖和限制](mot_sql_coverage_and_limitations.md)）。自openGauss 3.1.1版本以来，在单查询（如JOIN）的MOT表和堆表（基于磁盘的普通表）以及多步骤和多表事务中，MOT还支持MVCC和跨引擎事务（Cross-TX）。
...
- 隔离性（Isolation）：事务之间不能相互干扰。MOT支持可重复读、读已提交和快照隔离级别。更多信息，请参见[MOT隔离级别](mot_concurrency_control_mechanism.md)。
