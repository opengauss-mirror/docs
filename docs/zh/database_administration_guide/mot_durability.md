...
- **组同步重做日志记录**
    ...
    3. 当特定一组事务满足配置条件后，如[重做日志（MOT）](mot_configuration.md#section361563811235)小节中描述的已提交的事务数或超时时间，该组中的事务将被写入磁盘的WAL中。当这些日志条目被写入日志时，发出提交请求的客户端应用程序正在等待响应。
...
>[!NOTE]说明
>有关配置的详细信息，请参阅[重做日志（MOT）](mot_configuration.md#section361563811235)。
