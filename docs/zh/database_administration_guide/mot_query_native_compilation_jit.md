...
您可以参考MOT SQL覆盖和限制中的[不支持的JIT功能](mot_sql_coverage_and_limitations.md#section4815162910417)。

## JIT存储过程

JIT存储过程（JIT SP）由openGauss MOT引擎（从5.0版本开始）支持，其目标是提供更高的性能和更低的延迟。有关更多详细信息，请参阅[JIT存储过程](#jit存储过程jit-sp)。
