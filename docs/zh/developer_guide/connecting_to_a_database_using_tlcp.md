...
1. 以依赖于tassl的方式进行数据库编译，才能够支持服务端TLCP能力。具体内容请参考[编译前准备](../compilation_guide/setting_up_the_compilation_environment.md)。
2. 生成TLCP相关的证书/密钥。具体内容请参考[证书生成_TLCP](../database_administration_guide/ssl_certificate_management.md)。
3. 配置TLCP相关参数，重启数据库集群后生效。具体内容请参考[用SSL进行安全的TCP-IP连接](../database_administration_guide/establishing_secure_tcp_ip_connections_in_ssl_mode.md)。
...
1. 将客户端密钥转化为DER格式的方法不变，请参考[连接数据库_以SSL方式](./connecting_to_a_database_using_ssl_jdbc.md)。
