...
| `GR_CM_SO_NAME`                 | 空                    | `GS_TYPE_VARCHAR`   | 只读          | -                       | 重启生效         | CM共享库名称                  |
| `LOG_HOME`                      | 空                    | `GS_TYPE_VARCHAR`   | 可修改        | -                       | 重启生效         | 日志文件存储路径              |
| `LOG_BACKUP_FILE_COUNT`         | `20`                  | `GS_TYPE_INTEGER`   | 无属性        | `[0,128]`               | 重启生效         | 日志备份文件最大数量          |
| `LOG_MAX_FILE_SIZE`             | `256M`                | `GS_TYPE_INTEGER`   | 无属性        | `[1M,4G]`               | 重启生效         | 单个日志文件最大大小          |
| `INST_ID`                       | `0`                   | `GS_TYPE_INTEGER`   | 只读          | `[0,64)`                | 重启生效         | 实例ID                        |
| `LOG_LEVEL`                     | `519`                 | `GS_TYPE_INTEGER`   | 无属性        | `[0,4087]`              | 立即生效         | 日志级别                      |
| `MAX_SESSION_NUMS`              | `8192`                | `GS_TYPE_INTEGER`   | 只读          | `[16,16320]`            | 重启生效         | 最大会话数                    |
| `AUDIT_BACKUP_FILE_COUNT`       | `20`                  | `GS_TYPE_INTEGER`   | 无属性        | `[0,128]`               | 重启生效         | 审计日志备份文件最大数量      |
| `AUDIT_MAX_FILE_SIZE`           | `256M`                | `GS_TYPE_INTEGER`   | 无属性        | `[1M,4G]`               | 重启生效         | 单个审计日志文件最大大小      |
| `LOG_FILE_PERMISSIONS`          | `600`                 | `GS_TYPE_INTEGER`   | 只读          | `[600-777]`             | 重启生效         | 日志文件权限                  |
| `LOG_PATH_PERMISSIONS`          | `700`                 | `GS_TYPE_INTEGER`   | 只读          | `[700-777]`             | 重启生效         | 日志路径权限                  |
| `SSL_PWD_CIPHERTEXT`            | 空                    | `GS_TYPE_VARCHAR`   | 只读          | -                       | 重启生效         | SSL密码密文                  |
| `SHM_KEY`                       | `1`                   | `GS_TYPE_INTEGER`   | 只读          | `[1,64]`                | 重启生效         | 共享内存键值                  |
| `GR_NODES_LIST`                 | `0:127.0.0.1:1611`    | `GS_TYPE_VARCHAR`   | 无属性        | -                       | 立即生效         | oGRecorder节点列表            |
...
| `MES_WORK_THREAD_COUNT`         | `8`                   | `GS_TYPE_INTEGER`   | 只读          | `[2,64]`                | 重启生效         | MES工作线程数                 |
...
| `AUDIT_LEVEL`                   | `1`                   | `GS_TYPE_INTEGER`   | 无属性        | `[0,255]`               | 立即生效         | 审计日志级别                 |
...
| `DATA_FILE_PATH`                | `/tmp`                | `GS_TYPE_VARCHAR`   | 只读          | -                       | 重启生效         | 数据文件存储路径            |
