...
- 关于物理备份与恢复的更多说明，请参考[物理备份与恢复](../database_om_guide/physical_backup_and_restoration.md)。
...
- --with-encryption
    ...
    指定加密算法只支持使用第三方接口库，支持AES128_CBC、AES128_CTR、AES128_GCM、AES256_CBC、AES256_CTR、AES256_GCM、SM4_CBC、SM4_CTR、AES128_CBC_HMAC_SHA256、AES128_CTR_HMAC_SHA256、AES128_GCM_HMAC_SHA256、AES256_CBC_HMAC_SHA256、AES256_CTR_HMAC_SHA256、AES256_GCM_HMAC_SHA256、SM4_CBC_HMAC_SM3、SM4_CTR_HMAC_SM3。如果不带HMAC，表示只做加密。带HMAC表示需要带HMAC运算做完整性校验。

- --with-key

    指定加密密钥,只支持第三方接口库，第三方接口库使用的base64编码，最长44字节。不能和--gen-key同时指定。
...
- --instance-id
  ...
  数据库节点id号，因为资源池化模式只支持主机备份，因此该参数一般为0。在日志合一版本下（openGauss 7.0.0-RC1及之后的版本），该参数废弃。
...
## cm工具管理集群增量恢复流程（资源池化模式）
   ...
   **说明：** 当恢复的集群相当于备份来讲重新安装过或者不是原来的集群时，需要参考上面全量备份恢复流程中第7步和第10步，替换集群间认证的证书。
...
8. 进入主机恢复后的dn目录，创建恢复配置文件recovery.conf，指定数据库恢复的程度。
   ...
   >
   >- restore_command：该参数是将归档的XLOG复制到pg_xlog文件中，此参数不支持加--remove-destination，/mnt/server/archivedir为归档的XLOG路径且不支持使用相对路径，可以参考[dsscmd](../tool_and_commandreference/dsscmd.md)。
