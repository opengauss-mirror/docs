...
- 创建自认证证书
    ...
    自认证证书生成请参考（[证书生成](../database_administration_guide/ssl_certificate_management.md)\)。

    对私钥密码的保护，使用cm encrypt工具，请参考[cm\_ctl工具介绍](cm_ctl.md)。
...
3. 证书有效期的检测周期为1天，可通过ssl\_cert\_expire\_check\_interval设置。证书有效期剩余90天时会开始产生告警，可通过ssl\_cert\_expire\_alert\_threshold设置。（配置参数详见[ssl\_cert\_expire\_alert\_threshold](parameters_related_to_cm_server.md#section8148443183416)\)。
