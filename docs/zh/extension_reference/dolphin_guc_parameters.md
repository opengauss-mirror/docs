...
- disable_escape_bytea：是否一直使用对16进制进行解释，如果参数开启，等同与standard_conforming_strings对0x开头的16进制不起效，\x开头的字符串会当作一个普通的文本字符串来处理，不会被尝试转换成16进制数据，作用如下所示：
...
5. [#](dolphin_commenting_operators.md)
