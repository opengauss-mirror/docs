...
- **WITH \[ RECURSIVE \] with\_query \[, ...\]**
  ...
  - **plan\_hint子句**
    ...
    以/\*+ \*/的形式在INSERT关键字后，用于对INSERT对应的语句块生成的计划进行hint调优，详细用法请参见章节[使用Plan Hint进行调优](https://docs.opengauss.org/zh/docs/latest/performance_tuning_guide/plan_hint_optimization_overview.html)。每条语句中只有第一个/\*+ plan\_hint \*/注释块会作为hint生效，里面可以写多条hint。
...
- **partition\_clause**
  ...
  示例详见[CREATE TABLE SUBPARTITION](create_table_subpartition.md)

- **target\_query**

    作为插入目标的子查询，相当于视图，仅在参数sql\_compatibility=A时生效。插入限制详见[CREATE VIEW](create_view.md)中可自动更新视图一节。
    
- **WITH [ CASCADED | LOCAL ] CHECK OPTION**
    
    详见[CREATE VIEW](create_view.md)一节介绍。
