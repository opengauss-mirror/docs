...
- **plan\_hint子句**
    ...
    以/\*+ \*/的形式在UPDATE关键字后，用于对UPDATE对应的语句块生成的计划进行hint调优，详细用法请参见章节[使用Plan Hint进行调优](https://docs.opengauss.org/zh/docs/latest/performance_tuning_guide/plan_hint_optimization_overview.html)。每条语句中只有第一个/\*+ plan\_hint \*/注释块会作为hint生效，里面可以写多条hint。
...
- **target\_query**
    ...
    作为更新目标的子查询，相当于视图，仅在参数sql\_compatibility=A时生效。更新限制详见[CREATE VIEW](create_view.md)中可自动更新视图一节。
