...
- gs\_index\_advise\(text\)
    ...
    示例请参见[单query索引推荐](../characteristic_description/advanced_features/index_recommendation.md)。
...
- hypopg\_create\_index\(text\)
    ...
    示例请参见[虚拟索引](../characteristic_description/advanced_features/index_recommendation.md)。
...
- db4ai\_predict\_by\_bool \(text, VARIADIC "any"\)
    ...
    描述：获取返回值为布尔型的模型进行模型推断任务。此函数为内部调用函数，建议直接使用语法[PREDICT BY](predict_by.md)进行推断任务。
...
- db4ai\_predict\_by\_float4\(text, VARIADIC "any"\)
  ...
  描述：获取返回值为float4的模型进行模型推断任务。此函数为内部调用函数，建议直接使用语法[PREDICT BY](predict_by.md)进行推断任务。
...
- db4ai\_predict\_by\_float8\(text, VARIADIC "any"\)
  ...
  描述：获取返回值为float8的模型进行模型推断任务。此函数为内部调用函数，建议直接使用语法[PREDICT BY](predict_by.md)进行推断任务。
...
- db4ai\_predict\_by\_int32\(text, VARIADIC "any"\)
  ...
  描述：获取返回值为int32的模型进行模型推断任务。此函数为内部调用函数，建议直接使用语法[PREDICT BY](predict_by.md)进行推断任务。
...
- db4ai\_predict\_by\_int64\(text, VARIADIC "any"\)
  ...
  描述：获取返回值为int64的模型进行模型推断任务。此函数为内部调用函数，建议直接使用语法[PREDICT BY](predict_by.md)进行推断任务。
...
- db4ai\_predict\_by\_numeric\(text, VARIADIC "any"\)
  ...
  描述：获取返回值为numeric的模型进行模型推断任务。此函数为内部调用函数，建议直接使用语法[PREDICT BY](predict_by.md)进行推断任务。
...
- db4ai\_predict\_by\_text\(text, VARIADIC "any"\)
  ...
  描述：获取返回值为字符型的模型进行模型推断任务。此函数为内部调用函数，建议直接使用语法[PREDICT BY](predict_by.md)进行推断任务。
...
- gs\_explain\_model\(text\)
  ...
  示例请参见[CREATE MODEL](create_model.md)。
