...
该参数属于SUSET类型参数，仅支持[表1](../database_administration_guide/reset_parameters.md#zh-cn_topic_0283137176_zh-cn_topic_0237121562_zh-cn_topic_0059777490_t91a6f212010f4503b24d7943aed6d846)中的方式三进行设置。
...
**参数说明**： 指定是否允许一个只读串行事务延迟执行，使其不会执行失败。该参数设置为on时，当一个只读事务发现读取的元组正在被其他事务修改，则延迟该只读事务直到其他事务修改完成。该参数为预留参数，该版本不生效。与该参数类似的还有一个[default\_transaction\_deferrable](statement_behavior.md#zh-cn_topic_0237124732_zh-cn_topic_0059779117_s031e2bbea2cf42a5a1c24e6dbaa70365)，设置它来指定一个事务是否允许延迟。
...
该参数属于SIGHUP类型参数，改请参考[表2](../database_administration_guide/reset_parameters.md#zh-cn_topic_0283137176_zh-cn_topic_0237121562_zh-cn_topic_0059777490_t290c8f15953843db8d8e53d867cd893d)进行设置
...
## atf\_task\_counter\_timeout_sec<a name="section12345678901234"></a>

**参数说明**：设置应用透明故障转移（ATF）中所有可能的应用连接恢复超时时间，单位为秒。当设置此参数不为0时，集群开启应用透明故障转移（ATF）功能，集群在发生故障转移后，会在新主节点重建故障发生时正在连接的会话，在此期间，集群拒绝新事务的执行，只有在重建完成后等待一个连接恢复超时时间，新事务才能执行。

该参数属于POSTMASTER类型参数，请参考[表1](../database_administration_guide/reset_parameters.md#zh-cn_topic_0283137176_zh-cn_topic_0237121562_zh-cn_topic_0059777490_t91a6f212010f4503b24d7943aed6d846)中对应设置方法进行设置。

**取值范围**：整数，0 ~ 2147483647
- 0：不启用超时控制
- 大于0：连接恢复的最大超时时间
