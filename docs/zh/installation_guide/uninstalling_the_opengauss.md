...
2. 查看root用户互信是否建立，如果root用户没有建立互信，需要手工建立root用户互信，操作请参考[手工建立互信](initializing_the_installation_environment.md)。
...
5. 删除openGauss数据库各节点root用户的互信，操作参考[手工建立互信](initializing_the_installation_environment.md)的删除root用户互信部分内容。
