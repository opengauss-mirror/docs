...
openGauss是一款全面友好开放，携手伙伴共同打造的企业级开源多模数据库。openGauss提供结构化过滤、知识图谱检索、全文索引检索、向量检索四位一体的混合查询能力。openGauss提供面向多核架构的极致性能、全链路的业务、数据安全、基于AI的调优和高效运维的能力。openGauss深度融合华为在数据库领域多年的研发经验，结合企业级场景需求，持续构建竞争力特性。openGauss网站 \([https://opengauss.org/zh/](https://opengauss.org/zh/)  \) 提供了有关openGauss软件的最新信息。
...
- **openGauss数据库是多模的。**
    ...
    多模数据库是一种支持多种数据模型的数据库系统，它不仅支持传统的关系模型（即以行和列组成的二维表格结构），还同时支持其他类型的数据模型，如：

    - 全文搜索模型：用于处理文本数据，支持关键字搜索、分词、相关性排序等功能。
    - 向量搜索模型：用于处理高维向量数据，支持基于相似度的检索，广泛应用于图像识别、推荐系统、自然语言处理等场景。
    - 图搜索模型：用于表达实体之间的复杂关系，支持图遍历、路径查询、邻接分析等操作，适用于社交网络、知识图谱等领域。
...
- **openGauss数据库具有高性能、高可用、高安全、易运维、全开放的特点。**
    ...
    - 易运维
        ...
        - 提供在线自学习的SQL时间预测。
        - DataKit支持智能诊断插件，支持慢SQL根因分析，支持SQL诊断。
        - DataKit支持实例监控、告警监控、基础运维、智能参数调优、日志检索等插件，全方位支撑日常运维。

    - 创新架构
        - DataPod资源池化架构。主备机共享一份存储，实现基于磁阵设备的资源池化HA部署形态，减少存储所需容量。
        - DataVec向量数据库插件。支撑精确和近似的最近邻搜索、L2距离&余弦距离&内积、向量索引、向量操作函数和操作符等功能。
        - 通过dolphin插件，兼容大部分MySQL语法、函数、协议。
