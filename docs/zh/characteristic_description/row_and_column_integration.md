...
- 数据类型：参考[列存表支持的数据类型](../sql_reference/data_types_supported_by_column_store_tables.md)。特别地：
...
- 其他
    ...
    - 已行列转换的表，不支持 truncate 表、修改列名/列属性、增加/删除列、修改表压缩参数、不支持B库修改表字符集操作。特别地，已经转换的分区表，不支持新增、删除、交换、清空、分割、合并、移动、重命名操作。
