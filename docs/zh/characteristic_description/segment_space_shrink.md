...
- **[gs_table_shrink(新增)](../sql_reference/segment_page_storage_functions.md)**：回收段尾部空闲 extent，减少表空间膨胀。
- **[gs_space_shrink_compact(新增)](../sql_reference/segment_page_storage_functions.md)**：将物理文件中文件尾部的正在使用extent移动至前部（挤气泡），使得文件整理排布紧凑，并刷新高水位线，在刷新后高水位满足文件大小规律时执行物理文件收缩（128M对齐）。
