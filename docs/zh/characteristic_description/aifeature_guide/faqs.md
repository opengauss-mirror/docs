...
3. 上述命令执行完后，会要求用户输入私钥密码，用户按照要求输入密码后，会生成私钥文件private.key和证书文件certificate.crt。其中private.key中的内容以"-----BEGIN ENCRYPTED PRIVATE KEY-----"开头，表示私钥已加密。用户可将生成的文件地址配置在[service子命令](service.md)中WEB-SERVICE部分。
