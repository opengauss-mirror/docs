# 常见问题<a name="ZH-CN_TOPIC_0000002293246437"></a>
...
3. 上述命令执行完后，会要求用户输入私钥密码，用户按照要求输入密码后，会生成私钥文件gaussmaster.key和证书文件gaussmaster.crt。其中gaussmaster.key中的内容以"-----BEGIN ENCRYPTED PRIVATE KEY-----"开头，表示私钥已加密。用户可将生成的文件地址配置在[配置文件](generating_a_configuration_file.md)中WEB-SERVICE章节。
