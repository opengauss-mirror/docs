...
CM支持gstor的归档日志自动清理（7.0.0RC2引入）

- 添加gstor的归档日志自动清理功能，当gstor的归档日志大小超过阈值（2G）的85%时，触发自动清理归档日志策略，清理后保留阈值的15%大小。

CM支持显示灵衢场景下UBSE集群内存详情 （7.0.0RC2引入）

- 添加显示MXE集群内存功能，返回实时内存总量、借入借出以及可借用内存大小。
