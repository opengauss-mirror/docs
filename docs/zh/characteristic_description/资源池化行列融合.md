...
资源池化 HTAP 行列融合特性支持用户针对全表、表指定列进行行列转换及清除已有列缓存操作，不支持分区表。相关参数说明请参考[资源池化行列融合参数](../database_reference/资源池化行列融合参数.md)，开启多机并行查询请参考[spqplugin使用说明](../extension_reference/spqplugin使用.md)。
