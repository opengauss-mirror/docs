...
7. 全量迁移断点续传
   ...
   datakit执行Mysql全量迁移过程中可能会失败，再次迁移会重新迁移所有表，在迁移表数量过大或者数据过多的业务场景中，全量迁移耗时较长，迁移效率很低。为了提升迁移效率，在全量迁移失败的业务场景中，应提供断点续传功能，在重新迁移过程中跳过已经迁移完成的表。    
   基于以上考虑，对全量迁移功能进行了增强，前端提供“继续迁移”按钮，在全量迁移或者数据校验失败的场景下通过点击该按钮，后端可实现断点续传。该功能可用于离线迁移和在线迁移的全量迁移过程，增量迁移和反向迁移过程中失败，该功能不可用。
   1. 全量迁移失败
      Mysql全量迁移过程中失败，前端显示“继续迁移”按钮。
   
      ![img.png](figures/断点续传_全量迁移.png)
 
   2. 断点续传
      点击“继续迁移”按钮进行迁移，后端会使用断点续传功能，跳过已经迁移成功的表，只迁移未完成的表。
   
      ![img.png](figures/断点续传_全量迁移2.png)
      ![img.png](figures/断点续传_全量迁移3.png)
