...
MOT is automatically deployed as part of openGauss. You may refer to the  [MOT Preparation](mot_prerequisites.md)  section for a description of how to estimate and plan required memory and storage resources in order to sustain your workload. The  [MOT Deployment](mot_server_optimization.md)  section describes all the configuration settings in MOT, as well as non-mandatory options for server optimization.
