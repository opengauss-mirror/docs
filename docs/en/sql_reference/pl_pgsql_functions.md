...
For details about the PL/pgSQL syntax for creating functions, see  [CREATE FUNCTION](create_function.md). PL/pgSQL is a loadable procedural language. Its application method is similar to that of  [Stored Procedure](stored_procedures.md). There is only one difference. Stored procedures have no return values but the functions have.
