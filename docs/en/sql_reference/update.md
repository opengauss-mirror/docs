...
- **target\_query**
    ...
    Specified subquery for updates，which is equivalent to a view. This parameter takes effect only when **sql\_compatibility** is set to **A**. For restrictions on updating a subquery, see Automatically Updatable View section in [CREATE VIEW](create_view.md).
