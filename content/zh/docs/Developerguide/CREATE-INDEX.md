...
-   **COMPRESS\_LEVEL**
    ...
    索引参数，设置索引压缩算法等级，仅当COMPRESS_TYPE为2时生效。压缩等级越高，索引的压缩效果越好，索引的访问速度越慢。（仅支持B-TREE索引）
...
-   **compress_byte_convert**
    ...
    索引参数，设置索引压缩字节转换预处理。在一些场景下可以提升压缩效果，同时会导致一定性能力劣化。

    取值范围：布尔值，默认关闭。

-   **compress_diff_convert**

    索引参数，设置索引压缩字节差分预处理。只能与compress_byte_convert一起使用。在一些场景下可以提升压缩效果，同时会导致一定性能力劣化。
