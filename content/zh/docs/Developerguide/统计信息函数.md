...
-   DBE_PERF.standby_statement_history(bool \[, time1, time2\])
    ...
    参数：bool only_slow：是否仅查询慢SQL，true为是，false或NULL为查询全部SQL。VARIADIC timestamptz finish_time: 可选输入，最多输入两个时间time1, time2，表示查询的SQL的finish_time所属时间段。
    ...
    > 
    > - 两个时间参数time1 time2的说明：表示查询的SQL的finish_time所属时间段, 分别表示起始与终止时间, 输入NULL或者不输入表示没有限制, 功能等同与 select .. where finish_time between time1 and time2.备机上数据并非存在表里，不存在start_time列的索引，推荐使用参数对finish_time进行条件查找，内部有对这进行一些性能的优化。但注意修改了系统时间可能会导致此功能不准确。
    > 
