...
-   pg\_stat\_get\_activity\(integer\)
    ...
    ```
    openGauss=# select * from pg_stat_get_activity(140036483839744);
...
>
  > 此函数中所说节点指分布式节点，当前openGauss为集中式数据库。global与local功能一致，仅在当前节点生效，取值不支持BY\_CNID。
...
-   gs\_wlm\_session\_respool\(bigint\)
    ...
    描述：获取当前所有后台线程的session resouce pool相关信息。入参为bigint类型，可以为任意bigint值或NULL。
