...
-   insert(str,pos,len,newstr)
    ...
    > ![](public_sys-resources/icon-note.gif) **说明：**如果pos不在字符串的长度范围内，则返回原始字符串。如果len不在字符串的其余部分的长度范围内，从位置pos替换字符串的其余部分。如果参数为NULL返回NULL。
