...
>
>- openGauss支持多种类型的数据库，目前有4种，分别是Oracle类型，MySQL类型，Teradata类型以及Postgresql类型。在不指定数据库类型时，我们的数据库默认是oracle类型，oracle的词法分析器与另外三种不一样，在oracle中空字符串会被当作是NULL。所以，当使用oracle类型的数据库时，假如上述字符操作函数中有空字符串作为参数，会出现没有输出的情况。如：
>
>  ```
...
>  ```
>
>  这是因为内核在调用相应的函数进行处理前，会判断所输入的参数中是否含有NULL，假如有，则不会调用相应的函数，因此会没有输出。
>
>
>
> 
