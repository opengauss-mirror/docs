...
针对逻辑复制的配置选项，除了在[逻辑解码](zh-cn_topic_0289900774.md)中的配置选项外，还有专门给JDBC流式解码增加的配置项，如下所示：
...
2.  解码格式
    ...
    通过配置选项decode-style，指定解码格式。其取值为char型的字符'j'、't'或'b'，分别代表json格式、text格式及二进制格式。默认值为'b'即二进制格式解码。该选项仅允许并行解码时设置。
...
在并行解码的标准场景下（16核CPU、内存128G、网络带宽 \> 200MBps、表的列数为10\~100、单行数据量0.1k\~1k、DML操作以insert为主、不涉及落盘事务即单个事务中语句数量小于4096、parallel\_decode\_num为8、解码格式为'b'且开启批量发送功能），解码性能（这里以xlog消耗量为标准）不低于100MBps。
