...
-   由于部分数据节点状态不正常，导致集群启动失败。
-   目录权限不够。例如对/tmp目录、集群数据目录的权限不足。
-   配置的端口已经被占用。
-   开启了系统防火墙导致集群启动失败。
-   组成集群数据库的各台机器之间需要正确建立互信关系，在互信关系出现异常的情况下，数据库将无法启动。
...
-   确认是否由于实例状态不正常，导致集群启动失败。 通过gs\_om -t status --detail工具，查询当前集群各主备机实例的状态。

    -   如果某一节点上的所有实例都异常，请进行主机替换。

    -   如果发现某一实例状态为Unknown、Pending和Down的状态，则以集群用户登录到状态不正常的实例所在节点，查看该实例的日志检查状态异常的原 因。例如：
...
-   目录权限不够处理办法。例如，对/tmp目录、集群数据目录的权限不足。

    -   根据错误提示，确认权限不足的目录名称。

    -   使用chmod命令修改目录权限使其满足要求。对于/tmp目录，数据库用户需要具有读写权限。对于集群数据目录，请参考权限无问题的同类目录进行设置。
...
-   确认是否由于互信关系出现异常，导致集群无法启动。 重新配置集群中各台机器的互信关系解决此问题。
