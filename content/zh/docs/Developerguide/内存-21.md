...
>-   若max\_process\_memory-shared\_buffers-cstore\_buffers-元数据少于2G，GaussDB KernelopenGauss强制把enable\_memory\_limit设置为off。其中元数据是GaussDB KernelopenGauss内部使用的内存，和部分并发参数，如max\_connections，thread\_pool\_attr，max\_prepared\_transactions等参数相关。
...
320GB（60核CPU/480G内存）；160GB（32核CPU/256G内存）；80GB（16核CPU/128G内存）；40GB（8核CPU/64G内存）
...
**参数说明：**设置GaussDB KernelopenGauss使用的共享内存大小。增加此参数的值会使GaussDB KernelopenGauss比系统默认设置需要更多的System V共享内存。
...
160GB（60核CPU/480G内存）；80GB（32核CPU/256G内存）；40GB（16核CPU/128G内存）；20GB（8核CPU/64G内存）
...
128GB（60核CPU/480G内存，32核CPU/256G内存，16核CPU/128G内存）；64MB（8核CPU/64G内存）
...
2GB（60核CPU/480G内存）；1GB（32核CPU/256G内存）；512MB（16核CPU/128G内存）；256MB（8核CPU/64G内存）
...
>-   如果设置此参数的值大于实际的内核限制，则一个正在运行的递归函数可能会导致一个独立的服务器进程崩溃。在GaussDB KernelopenGauss能够检测内核限制的操作系统上，将自动限制设置为一个不安全的值。
