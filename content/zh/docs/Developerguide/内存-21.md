...
>-   当[自动清理](自动清理.md)进程运行时，autovacuum\_max\_workers倍数的内存将会被分配，所以此时设置maintenance\_work\_mem的值应该不小于work\_mem。
...
>-   如果设置此参数的值大于实际的内核限制，则一个正在运行的递归函数可能会导致一个独立的服务器进程崩溃。在openGauss能够检测内核限制的操作系统上，将自动限制设置为一个不安全的值。
