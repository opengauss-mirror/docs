...
3.  查看系统中长时间运行的查询语句。
    ...
    如果当前系统较为繁忙，可以使用[TIMESTAMPDIFF](zh-cn_topic_0289900496.md#zh-cn_topic_0283136846_section5629194495516)函数通过限制current\_timestamp和query\_start大于某一阈值查看执行时间超过此阈值的查询语句。timestampdiff的第一个参数为时间差单位。例如，执行超过2分钟的查询语句可以通过如下语句查询。
