...
**默认值：**单机模式下默认值为off，一主多备的模式下默认值为on
...
**参数说明：**设置用于存放WAL数据的共享内存空间的XLOG\_BLCKSZ数，XLOG\_BLCKSZ的大小默认为8KB。实际设置结果为共享内存空间的大小。

该参数属于POSTMASTER类型参数，请参考[表1](zh-cn_topic_0289899927.md#zh-cn_topic_0283137176_zh-cn_topic_0237121562_zh-cn_topic_0059777490_t91a6f212010f4503b24d7943aed6d846)中对应设置方法进行设置。

**取值范围：**-1\~2<sup>18</sup>，即（-1\~2<sup>18</sup>）* 8 KB
...
## xlog\_idle\_flushes\_before\_sleep<a name="section613712442389"></a>

**参数说明：**设置WAL Writer线程在进入睡眠前的空刷xlog次数，该值太大会导致空闲时CPU过高。

该参数属于POSTMASTER类型参数，请参考[表1](zh-cn_topic_0289899927.md#zh-cn_topic_0283137176_zh-cn_topic_0237121562_zh-cn_topic_0059777490_t91a6f212010f4503b24d7943aed6d846)中对应设置方法进行设置。

**取值范围：**整型，0\~INTMAX，单位为次。

**默认值：**500

## wal\_writer\_cpu<a name="section16468181013441"></a>

**参数说明：**指定用于WAL Writer线程绑定的CPU，若为-1，不进行绑核。
...
>![](public_sys-resources/icon-notice.gif) **须知：**  
>绑核步骤如下：  
>1.  查找xlog所在盘与NUMA节点的亲和性，例如，假设xlog日志存放在/dev/nvme4上，使用cat /sys/class/nvme/nvme4/device/numa\_node命令获取NUMA节点号。  
>2.  设置该GUC参数指定绑定的CPU为该NUMA节点上的某一个工作线程可用的CPU（非中断、网卡绑定CPU\)。  
>3.  设置GUC参数thread\_pool\_attr，将上述已绑定的CPU从线程池可用的CPU集合中删除。  
>4.  对每个数据节点重复上述步骤。  

## xlog\_flush\_uplimit<a name="section119411424917"></a>

**参数说明：**设置xlog触发flush的字节数上限。

该参数属于POSTMASTER类型参数，请参考[表1](zh-cn_topic_0289899927.md#zh-cn_topic_0283137176_zh-cn_topic_0237121562_zh-cn_topic_0059777490_t91a6f212010f4503b24d7943aed6d846)中对应设置方法进行设置，不推荐用户修改此设置。

**取值范围：**整型，100\~INTMAX，单位为Byte。

**默认值：**INTMAX

## wal\_file\_init\_num<a name="section19488046876"></a>

**参数说明：**设置WAL Auxiliary Thread预创建的xlog文件数量。
