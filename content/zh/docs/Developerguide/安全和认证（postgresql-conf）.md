...
**取值范围：**字符串，如果指定多个加密算法，加密算法之间需要以分号分割。详细请参见[表4](用SSL进行安全的TCP-IP连接.md#zh-cn_topic_0237121092_zh-cn_topic_0059778374_t34eea0830ef94be1a866f0410ba3eb07)获取支持的加密算法。
...
**参数说明：**指定Kerberos服务主配置文件的位置，详细请参见[客户端接入认证](客户端接入认证.md)。
...
**参数说明：**设置Kerberos服务名，详细请参见[客户端接入认证](客户端接入认证.md)。
