...
openGauss提供了一个隐式定义的拥有所有角色的组PUBLIC，所有创建的用户和角色默认拥有PUBLIC所拥有的权限。关于PUBLIC默认拥有的权限请参考[GRANT](zh-cn_topic_0289900312.md)。要撤销或重新授予用户和角色对PUBLIC的权限， 可通过在GRANT和REVOKE指定关键字PUBLIC实现。
...
非[三权分立](zh-cn_topic_0289900233.md)时，只有系统管理员和具有CREATEROLE属性的用户才能创建、修改或删除角色。三权分立下，只有初始用户和具有CREATEROLE属性的用户才能创建、修改或删除角色。

-   要创建角色，请使用[CREATE ROLE](zh-cn_topic_0289900481.md)。
-   要在现有角色中添加或删除用户，请使用[ALTER ROLE](zh-cn_topic_0289900826.md)。
-   要删除角色，请使用[DROP ROLE](zh-cn_topic_0289900731.md)。DROP ROLE只会删除角色，并不会删除角色中的成员用户帐户。
