...
-   gs\_space\_shrink\(int4 tablespace, int4 database, int4 extent\_type, int4 forknum\)
    ...
    描述：效果跟local\_space\_shrink类似，对指定段页式空间做物理空间压缩,但参数不同，传入的是tablespace和database的oid，extent\_type为\[2,5\]的int值。注意：extent\_type = 1表示段页式元数据，目前不支持对元数据所在的物理文件做压缩。该函数仅限工具使用，不建议用户直接使用。
