...
**参数说明：**延迟进入最大可用模式的时间
- 当最大可用模式most_available_sync配置为on时，在主备场景下，当存在同步备发生故障, 导致不满足当前所配置的同步备数量(详细可参考synchonous_standby_name的含义)时, 如果配置了keep_sync_window参数, 则在keep_sync_window设置的时间窗口内，继续保持最大保护模式，即阻塞主机的事务提交, 延缓进入最大可用模式的时间。
