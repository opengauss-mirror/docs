...
**参数说明：**打开最大可用模式most_available_sync，主备场景下，当同步备故障时，在keep_sync_window超时窗口内，继续保持最大保护模式，延缓进入最大可用模式。若在keep_sync_window超时窗口内，同步备机故障恢复，则不再进入最大可用模式。如果设置keep_sync_window，建议使用的最小值为5，以避免触发误报。
