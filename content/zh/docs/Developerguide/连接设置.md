...
**参数说明：**声明服务器监听客户端的TCP/IP地址。该参数指定openGauss服务器使用哪些IP地址进行监听，如IPV4或IPV6（若支持）。服务器主机上可能存在多个网卡，每个网卡可以绑定多个IP地址，该参数就是控制openGauss到底绑定在哪个或者哪几个IP地址上。而客户端则可以通过该参数中指定的IP地址来连接openGauss或者给openGauss发送请求。
