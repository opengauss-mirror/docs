...
该参数指定GaussDB KernelopenGauss服务器使用哪些IP地址进行侦听，如IPV4或IPV6（若支持）。服务器主机上可能存在多个网卡，每个网卡可以绑定多个IP地址，该参数就是控制GaussDB KernelopenGauss到底绑定在哪个或者哪几个IP地址上。而客户端则可以通过该参数中指定的IP地址来连接GaussDB KernelopenGauss或者给GaussDB KernelopenGauss发送请求。
...
**参数说明：**声明当前节点连接数据库openGauss其他节点绑定的本地IP地址。
...
**参数说明：**GaussDB KernelopenGauss服务侦听的TCP端口号。
...
**参数说明：**允许和数据库连接的工具的最大并发连接数。此参数会影响GaussDB KernelopenGauss的工具连接并发能力。
...
增大此参数可能导致GaussDB KernelopenGauss要求更多的SystemV共享内存或者信号量，可能超过操作系统缺省配置的最大值。这种情况下，请酌情对数值加以调整。
...
**参数说明：**设置GaussDB KernelopenGauss服务器侦听客户端连接的Unix域套接字目录。
...
>    ```
>    {"driver_name":"ODBC","driver_version": "(openGauss X.X.X build 13b34b53GaussDB Kernel V500R001C00 build 290d125f) compiled at 2020-05-08 02:59:43 commit 2143 last mr 131 debug","driver_path":"/usr/local/lib/psqlodbcw.so","os_user":"omm"}
>    ```
>    默认显示driver\_name和driver\_version，driver\_path和os\_user的显示由用户控制（参见[连接数据库](zh-cn_topic_0289900896.md)和[Linux下配置数据源](zh-cn_topic_0289900737.md)）。
