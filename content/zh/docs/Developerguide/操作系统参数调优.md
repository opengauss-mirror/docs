# 操作系统参数调优<a name="ZH-CN_TOPIC_0245374528"></a>
...
-   设置10GE网卡最大传输单元（MTU），使用ifconfig命令设置。10GE网卡推荐设置为8192，可提升网络带宽利用率。
    ...
    >在数据库节点配置参数设置comm\_tcp\_mode=false时，一定要保证各个openGauss的MTU大小相等，否则可能有通信问题。  
...
设置hugepage属性。在“/boot/grub/menu.lst”文件中第一个kernel行里追加下面参数，来关闭透明大页。
