...
-   pgxc\_lock\_for\_backup\(\)
    ...
    描述：为备份操作给数据库openGauss加锁，这些备份是为在新增节点上做恢复。
...
-   copy\_error\_log\_create\(\)
    ...
    >-   由于尝试创建的public.pgxc\_copy\_error\_log定义是一张行存表，因此数据库openGauss上必须支持行存表的创建才能够正常运行此函数，并使用后续的COPY容错功能。需要特别注意的是，enable\_hadoop\_env这个GUC参数开启后会禁止在数据库openGauss内创建行存表（openGauss默认为off）。
...
-   generate\_wdr\_report\(begin\_snap\_id Oid, end\_snap\_id Oid, int report\_type, int report\_scope, int node\_name\)
    ...
    **表 2**  generate\_wdr\_report 参数说明
    ...
    <td class="cellrowborder" valign="top" width="27.93%"><p id="zh-cn_topic_0283137465_p1121534790"><a name="zh-cn_topic_0283137465_p1121534790"></a><a name="zh-cn_topic_0283137465_p1121534790"></a>node：<span id="text17361182615149"><a name="text17361182615149"></a><a name="text17361182615149"></a>数据库openGauss</span>中的节点。</p>
    </td>
    </tr>
