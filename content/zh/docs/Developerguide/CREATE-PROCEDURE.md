# CREATE PROCEDURE<a name="ZH-CN_TOPIC_0242370574"></a>
...
-   如果存储过程参数中带有出参，SELECT调用存储过程必须缺省出参，CALL调用存储过程适配A，调用非重载函数时必须指定出参，对于重载的package函数，out参数可以缺省，具体信息参见[CALL](CALL.md)的示例。
