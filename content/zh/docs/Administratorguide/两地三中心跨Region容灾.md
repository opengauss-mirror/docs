...
- 灾备数据库实例未打开极致RTO功能，数据库实例可读不可写；灾备数据库实例打开极致RTO功能\(参见《特性描述》)，数据库实例不可读不可写。

- 灾备数据库实例通过failover命令升主后，和原主数据库实例灾备关系将失效，需要重新搭建容灾关系。

- 在主数据库实例和灾备数据库实例处于normal状态时可进行容灾搭建；在主数据库实例处于normal态并且灾备数据库实例已经升主的情况下，主数据库实例可执行容灾解除，其他数据库实例状态不支持。

  在主数据库实例和灾备数据库实例处于normal状态时，通过计划内switchover命令，主数据库实例可切换为灾备数据库实例，灾备数据库实例可切换为主数据库实例。

  灾备数据库实例处于非Normal且非Degraded状态时，无法升主，无法作为灾备数据库实例继续提供容灾服务，需要修复或重建灾备数据库实例。

- 容灾状态中灾备数据库实例不支持降副本，灾备数据库实例升主后正常支持升降副本。

- 该解决方案不支持DCF模式。

- 灾备数据库实例DN多数派故障或者CMS、数据库节点全故障，数据库实例处于非Normal且非Degraded状态时，无法执行流式容灾信息写入（主数据库实例），无法启动容灾（主备数据库实例），无法解除容灾（主数据库实例），灾备数据库实例无法升主，无法作为灾备数据库实例，需要重建灾备数据库实例。执行过降副本的数据库实例要作为灾备数据库实例参与容灾搭建，仍需遵从前述约束，可以在搭建前修复，也可以在完成搭建后进行修复。

- 当灾备数据库实例为2副本时，需要确保打开最大可用模式\(参数most\_available\_sync\)。灾备数据库实例在1个副本损坏时，仍可以升主对外提供服务，如果剩余的这个副本也损坏，将导致不可避免的数据丢失。

- 主数据库实例如果进行了强切操作\(cm\_ctl finishredo命令，参见《工具参考》)，需要重建灾备数据库实例。

- 建立容灾关系的主数据库实例与灾备数据库实例之间不支持GUC参数的同步。

- 容灾关系搭建之后，不支持数据库节点实例端口修改。

- 容灾关系搭建前，不支持已存在级联备。

- 建议对于流式容灾流复制IP的选择，应考虑尽量使集群内的网络平面与跨集群网络平面分离，便于压力分流并提高安全性。

- 在搭建前在主集群需创建具有流复制权限的用户。

- 容灾搭建时需要在主数据库实例和灾备数据库实例下发容灾用户名和密码用于数据库实例间鉴权：

  -   主备数据库实例必须使用相同的容灾用户名和密码。
  -   一次容灾搭建后，该用户密码不可修改。若需修改容灾用户名与密码，需要解除容灾，使用新的容灾用户重新进行搭建。
