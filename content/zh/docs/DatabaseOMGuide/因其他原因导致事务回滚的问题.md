...
```
## 三、问题根因及解决方案
如果事务内报错，则导致事务自动回滚，事务内的修改不会生效，保证数据一致性。事务内报错的原因可能很多，比如语法错误、会话长时间未执行操作导致超时、操作不存在的表等等。事务内报错的原因可能很多，需要具体分析，一方面可以通过客户端返回的报错信息识别报错原因。另一方面可以查看pg_log目录下的数据库日志里是否存在报错信息识别报错原因。
