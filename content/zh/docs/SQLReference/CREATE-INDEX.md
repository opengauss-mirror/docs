...
- **WITH \( \{storage\_parameter = value\} \[, ... \] \)**
    ...
    - **COMPRESSTYPE**
        ...
        索引参数，设置索引压缩算法。1 代表 pglz 算法（不推荐使用），2 代表 zstd 算法，默认不压缩。该参数生效后不允许修改。（仅支持 B-TREE 索引）

        取值范围：0\~2，默认值为 0。

    - **COMPRESS\_LEVEL**

        索引参数，设置索引压缩算法等级，仅当 COMPRESSTYPE 为 2 时生效。压缩等级越高，索引的压缩效果越好，索引的访问速度越慢。该参数允许修改，修改后影响变更数据、新增数据的压缩等级。（仅支持 B-TREE 索引）
