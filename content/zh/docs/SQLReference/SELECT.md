  ...
  - TIMECAPSULE \{ TIMESTAMP | CSN \} expression
    ...
    - join_type
      ...
      - CROSS JOIN
        ...
        > ![img](https://docs.opengauss.org/zh/docs/latest/docs/Developerguide/public_sys-resources/icon-note.gif) **说明：** 必须为INNER和OUTER连接类型声明一个连接条件，即NATURAL ON、join_condition、USING (join_column [， …]) 之一。但是它们不能出现在CROSS JOIN中。
