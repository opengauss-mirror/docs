...
- **FROM子句**
  ...
  - TIMECAPSULE \{ TIMESTAMP | CSN \} expression
    ...
    - TIMECAPSULE CSN
      ...
      > - 闪回点过旧时，因闪回版本被回收等导致无法获取旧版本会导致闪回失败，报错：Restore point too old。可通过将[version_retention_age](../DatabaseReference/审计.md#version_retention_age)和[vacuum_defer_cleanup_age](../DatabaseReference/闪回相关参数.md#vacuum_defer_cleanup_age)设置成同值，配置闪回功能旧版本保留期限，取值范围是0~1000000，值为0表示VACUUM不会延迟清除无效的行存记录。
