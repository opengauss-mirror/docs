...
- generate_procoverage_report<a name="section14683125861213"></a>
  ...
  描述：生成postgres库[coverage.proc_coverage](../SQLReference/coverage-proc_coverage.md)表中两个覆盖率序号间存储过程的覆盖率报告，需要在postgres库下执行，且打开enable_proc_coverage参数。
  ...
  >![](public_sys-resources/icon-note.png) **说明：**
  >
  > 1. 当begin_id和end_id符合取值范围时，生成二者间（包含）id的所有覆盖率记录的报告。如generate_procoverage_report(2, 5)，若[coverage.proc_coverage](../SQLReference/coverage-proc_coverage.md)表中存在coverage_id为2、3、4、5的报告，将生成2、3、4、5记录的覆盖率报告；若仅存在coverage_id为3、4的报告，将生成3、4记录的覆盖率报告。
