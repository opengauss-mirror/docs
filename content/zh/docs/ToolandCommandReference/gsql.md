...
使用gsql命令可以连接远程数据库服务。连接远程数据库服务时，需要在服务器上设置允许远程连接，详细操作请参见[使用gsql访问openGauss](../GettingStarted/使用gsql访问openGauss.md)。
...
-   gsql: FATAL:  Forbid remote connection with trust method!
    ...
    数据库由于安全问题，禁止远程登录时使用trust模式。这时需要修改pg\_hba.conf里的连接认证信息。具体的设置信息请参见[配置文件参考](../DatabaseAdministrationGuide/配置文件参考.md)章节。
