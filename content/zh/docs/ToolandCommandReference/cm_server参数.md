...
**参数说明：** cm\_server强起逻辑等待时延。在force\_promote被置为1时，当集群的某一分片处于无主状态开始计时，等待该延迟时间后开始执行强启逻辑。
