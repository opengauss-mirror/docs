# 全密态数据库等值查询<a name="ZH-CN_TOPIC_0000001088278190"></a>
...
-   当前版本只支持gsql客户端，暂不支持诸如JDBC/ODBC等其他客户端实现密态等值查询。
-   只支持通过客户端执行copy from stdin的方式、\\copy命令的方式以及insert into values\(…\)的方式往密态表中导入数据。
-   不支持包括排序、范围查询以及模糊查询等在内的除等值以外的各种密态查询。
-   不支持存储过程“procedure statement”语法。
-   不支持通过insert into…select…语法将非加密表数据插入到加密表数据中。
-   对于查询运算中如果参与比较的两个属性条件采用不同的数据加密密钥，返回报错。
-   密态等值查询支持的数据类型包括：
