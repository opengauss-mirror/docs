...
-   已按照[搭建编译环境](搭建编译环境.md)的要求准备好相关软硬件，并且已参考[代码下载](#编译前准备)下载了代码。
-   已完成开源软件编译构建，具体请参见[开源软件编译构建](#编译前准备)。
-   了解  [build.sh](#build.sh介绍)脚本的参数选项和功能。
...
2.  执行如下命令编译出openGauss产品安装包。
    ...
    本操作和[软件安装编译](#软件安装编译)相比，同样会经历的一键式编译最终生成软件的过程、与将软件封装成安装包的过程。对比[build.sh](#build.sh介绍)的使用命令可发现，此处仅增加了一个 '-pkg' 功能选项
