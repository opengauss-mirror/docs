...
1. 执行如下命令下载代码和开源第三方软件仓库：
   ...
   > 下载地址：https://opengauss.obs.cn-south-1.myhuaweicloud.com/3.0.0/openGauss-third_party_binarylibs.tar.gz
...
2.  <a name="li1666842982511"></a>执行脚本获取自己系统的版本。
    ...
    >![](public_sys-resources/icon-note.gif) **说明：** 
    > 
    >-   显示的结果表示openGauss当前支持的操作系统，openGauss支持的操作系统为centos7.6\_x86\_64、openeuler\_aarch64、openeuler\_x86\_64。  
    >
...
源码编译完openGauss-server代码仓之后，并没有gs_om。如需使用gs_om，需单独编译openGauss-OM，并将编译好的openGauss-xxx-om.tar.gz的包拷贝到openGauss-server安装包所在的目录，再按照[企业版]( https://docs.opengauss.org/zh/docs/3.0.0/docs/installation/%E4%BC%81%E4%B8%9A%E7%89%88%E5%AE%89%E8%A3%85.html )的方式进行安装。
