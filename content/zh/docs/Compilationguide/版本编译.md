...
如果您曾经安装过openGauss的旧版本，或者源码编译过openGauss，可按下列办法进行环境清理：
...
没有关闭swap分区，可能会导致在后续的编译过程中，执行make -j的时候报内存不足的错。可按一下方式关闭swap分区：

```
swapoff -a
```

可使用free -m查看swap分区是否关闭。
...
1.  执行如下命令下载代码和开源第三方软件仓库等，其中_\[git ssh xxx address\]_表示实际代码下载地址，可在openGauss社区获取这些地址。

    ```
    [user@linux sda]$ git clone [git ssh openGauss-server address] -b branchname
    [user@linux sda]$ git clone [git ssh openGauss-third_party address] -b branchname
    ...
    >-   branchname：代码分支名称，缺省可不填，如需编译openGauss 2.0.0的代码，可加上-b 2.0.0的参数。
    ...
    >  下载地址：https://opengauss.obs.cn-south-1.myhuaweicloud.com/2.0.0/openGauss-third\_party\_binarylibs.tar.gz
...
<td class="cellrowborder" valign="top" width="50%" headers="mcps1.2.3.1.2 "><p id="p13141735182220"><a name="p13141735182220"></a><a name="p13141735182220"></a>建议版本：1.9</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="50%" headers="mcps1.2.3.1.2 "><p id="p63616179237"><a name="p63616179237"></a><a name="p63616179237"></a>建议版本：3.19.2</p>
</td>
</tr>
...
2.  <a name="li1666842982511"></a>执行脚本获取自己系统的版本
...
5. 执行如下命令，编译安装。
   ...
   > -   如果使用-j参数，make执行的时候会一直增加并发数，有时候会遇到"cannot allicate memory"的错。一般-j参数后可指定数字为 cpu数 X 2。如编译环境分配了4个CPU，可使用make -j8的参数进行编译。
...
编译结束后，可按以下方面对编译后的openGauss进行验证:
...
5. 启动数据库：
   ...
   启动完毕后可通过 ps -ux | grep gaussdb检查数据库进程情况，或者通过 gs_ctl query -D /home/omm/data检查数据库状态，或使用 gsql -d postgres 进入gsql命令行查看数据库的相关信息。
...
常用的gs_om管理工具在[openGauss-OM代码仓](https://gitee.com/opengauss/openGauss-OM.git)内，如果openGauss-server已完成编译，如需使用gs_om，需要单独编译openGauss-OM，并将编译好的openGauss-xxx-om.tar.gz的包拷贝到openGauss-server安装包所在的目录，再按照[企业版]( https://opengauss.org/zh/docs/latest/docs/installation/%E4%BC%81%E4%B8%9A%E7%89%88%E5%AE%89%E8%A3%85.html )的方式进行安装。
