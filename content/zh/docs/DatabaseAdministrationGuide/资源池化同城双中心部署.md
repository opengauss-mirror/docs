...
-   操作步骤
    1. 在主存储上创建资源池化需要的lun，以及远程同步复制xlog卷对应的lun，并且所有lun全部映射到业务计算节点上
        ...
        <tr>
            <td rowspan='2'>xlog0</td>
        </tr>
            <td rowspan='1'>/dev/sdb</td>
        <tr>
            <td rowspan='2'>xlog1</td>
        </tr>
            <td rowspan='1'>/dev/adc</td>
        <tr>
            <td rowspan='2'>votingDiskPath</td>
        </tr>
            <td rowspan='1'>/dev/sdd</td>
        <tr>
            <td rowspan='2'>shareDiskDir</td>
        </tr>
            <td rowspan='1'>/dev/sde</td>
        <tr>
            <td rowspan='2'>远程同步复制xlog</td>
        </tr>
            <td rowspan='1'>/dev/adf</td>
        </tbody>
        </table>
    ...
    3.  在主存储上执行如下操作安装部署主集群，安装用户omm。
        ...
        su - omm
        source /home/omm/env
        gs_install -X /opt/software/openGauss/cluster_config.xml --dorado_config=/dev/sdf
        ```

        参数解释：
           + sep-env-file    分离环境变量，参数取值是一个安装用户omm可以访问到的文件目录
           + dorado_config   主存储上为远程同步复制xlog划分的lun，只取值主存储对应的远程同步复制xlog划分的lun对应的盘符
    ...
    6.  在备存储上集群(建立容灾关系之后就是备集群)上执行如下操作安装部署备集群,安装用户omm。
        ...
        ```
        `Tips`: 备存储上单集群安装部署gs_install不加dorado_config参数
    ...
    7.  查询备存储上集群(建立容灾关系之后就是备集群)状态。
        ...
        node           node_ip         instance                             state            | node           node_ip         instance                             state
        ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        1  node1 20.20.20.20   6001 25400  /opt/huawei/install/data/dn P Primary Normal | 2  node2 20.20.20.20   6002 25400  /opt/huawei/install/data/dn S Standby Normal
        ```
    ...
    12. 拉起首备dssserver，执行build
        ...
        dsserver -D /opt/huawei/install/dss_home &
