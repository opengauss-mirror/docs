...
在创建一个表后，表中并没有数据，在使用这个表之前，需要向表中插入数据。本小节介绍如何使用[INSERT](../SQLReference/INSERT.md)命令插入一行或多行数据，及从指定表插入数据。如果有大量数据需要批量导入表中，请参考[导入数据](导入数据.md)。
...
向表中插入数据前，意味着表已创建成功。创建表的步骤请参考[创建和管理表](创建和管理普通表.md)。
