...
请通过openGauss社区获取HCOM4DB和HCOM安装包。
...
<tr id="zh-cn_topic_0000002082250312_row4173848152519"><td class="cellrowborder" valign="top" headers="mcps1.2.4.1.1 "><p id="zh-cn_topic_0000002082250312_p148785361288"><a name="zh-cn_topic_0000002082250312_p148785361288"></a><a name="zh-cn_topic_0000002082250312_p148785361288"></a>OCK_Hcom4db_24.0.0_openeuler2203_aarch64.tar.gz</p>
</td>
<td class="cellrowborder" valign="top" headers="mcps1.2.4.1.2 "><p id="zh-cn_topic_0000002082250312_p2901951202814"><a name="zh-cn_topic_0000002082250312_p2901951202814"></a><a name="zh-cn_topic_0000002082250312_p2901951202814"></a>HCOM4DB安装包。</p>
</td>
</tr>
...
-   获取对应操作系统和CPU架构的HCOM4DB和HCOM安装包，例如：OCK\_Hcom4db\_24.0.0\_openeuler2203\_aarch64.tar.gz和BoostKit-hcom\_1.0.0\_openeuler2003\_aarch64.tar.gz。
-   主备节点已存在“$\{GAUSSHOME\}/lib“目录。

**操作步骤<a name="zh-cn_topic_0000002082095562_section16893155672411"></a>**

1.  以数据库管理用户omm登录节点，并上传安装包，上传目录仅omm用户有写权限。
2.  进入安装包上传目录，解压缩安装包。
    1.  新建HCOM4DB解压目录，解压缩HCOM4DB安装包。

        ```
        mkdir OCK_Hcom4db_24.0.0_openeuler2203_aarch64
        tar -xzvf OCK_Hcom4db_24.0.0_openeuler2203_aarch64.tar.gz -C OCK_Hcom4db_24.0.0_openeuler2203_aarch64 --no-same-owner
        ```
...
3.  拷贝相关so至“$\{GAUSSHOME\}/lib“目录。

    >![](./public_sys-resources/icon-note.png) **说明：** 
    >建议用户根据业务使用情况，设置so文件权限最小化。

    1.  将libhcom4db.so、libulog.so、libopenssl\_dl.so拷贝至“$\{GAUSSHOME\}/lib“。

        ```
        cp -f OCK_Hcom4db_24.0.0_openeuler2203_aarch64/lib/*.so ${GAUSSHOME}/lib/
        ```

    2.  删除librpc\_ucx.so（如果没有该文件则忽略），重命名libhcom4db.so为librpc\_ucx.so。

        ```
        rm -f ${GAUSSHOME}/lib/librpc_ucx.so
        mv ${GAUSSHOME}/lib/libhcom4db.so ${GAUSSHOME}/lib/librpc_ucx.so
        ```
...
4.  设置环境变量OCK\_RPC\_LIB\_PATH（librpc\_ucx.so的路径）和HCOM4DB\_LIB\_PATH（libhcom.so的路径）。
