...
- [准备软硬件安装环境](#准备软硬件安装环境)
  ...
  - [修改操作系统配置](#修改操作系统配置)
    ...
    - [设置字符集参数](#设置字符集参数)
    - [设置时区和时间](#设置时区和时间)
    - [（可选）关闭swap交换内存](#可选关闭swap交换内存)
    - [设置网卡MTU值](#设置网卡mtu值)
    - [关闭RemoveIPC](#关闭removeipc)
    - [关闭HISTORY记录](#关闭history记录)
...
<td class="cellrowborder" valign="top"pngdth="50%" headers="mcps1.2.3.1.2 "><p id="p1567262515496"><a name="p1567262515496"></a><a name="p1567262515496"></a>建议版本：2.28-36</p>
</td>
</tr>
</tbody>
</table>
...
使用date -s命令将各主机的时间设置为统一时间，举例如下。png
