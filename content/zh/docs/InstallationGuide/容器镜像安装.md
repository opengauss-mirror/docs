...
### 下载镜像包并加载
- 下载镜像包（以arm架构下对应的包为例），镜像包的架构应与主机架构匹配。x86_64镜像下载方式相同，镜像包获取路径为：https://opengauss.obs.cn-south-1.myhuaweicloud.com/6.0.5/openGauss-Docker-6.0.5-x86_64.tar
``` bash
$ wget https://opengauss.obs.cn-south-1.myhuaweicloud.com/6.0.5/openGauss-Docker-6.0.5-aarch64.tar
```
- 加载镜像
``` bash
$ docker load -i openGauss-Docker-6.0.5-aarch64.tar
```
- 验证镜像状态
```bash
$ docker images
