...
-   新增功能：
    ...
    -   [并行逻辑解码](并行逻辑解码.md)
    -   [CM（Cluster Manager）](统一数据库管理工具.md)
    -   [global syscache](支持global-syscache.md)
    -   [发布订阅](发布订阅.md)
    -   [外键锁增强](外键锁增强.md)
    -   [行存表压缩](CREATE-TABLE.md)
    -   [Data Studio工具开源](openGauss客户端工具DataStudio.md)
    -   [MySQL到openGauss的迁移工具chameleon](MySQL--openGauss迁移工具chameleon.md)
    -   [支持使用中间件shardingSphere构建分布式数据库](分布式数据库能力.md)
    -   [支持kubernetes部署分布式数据库](使用kubernetes部署分布式数据库.md)
    -   [支持ANY权限管理](GRANT.md)
    -   [DBMind组件化](AI4DB-数据库自治运维.md)
