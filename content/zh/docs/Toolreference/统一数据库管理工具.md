...
function exec_check
{
  ...
  if [ "$state" == "T" ]; then
    ...
    if [ "$firstphonytime" == "" ]; then
      ...
      #firstphonytime为用于保存当前资源实例假死时间的变量名称，
...
资源名称、资源binPath、用于过滤资源实例的命令关键词、用于保存首次检测到资源假死时间的文件（可选）、最长假死时间、记录首次假死时间的变量名（如果同一节点存在多个不同的自定义资源实例）
...
-   name: 自定义资源对象名称，字符串类型，最大长度为31
-   instances: 自定义资源所在节点列表
-   node_id: 资源实例所在节点的node_id
-   res_instance_id: 资源实例id，大于等于0
-   script: 资源脚本位置
-   check_interval: check时间间隔
-   time_out: 脚本执行的超时时间
