...
节点状态**NodeStatus**，对应链接为**http://ip:port/CMRestAPI/ClusterStatus[?nodeId=n]**，指定nodeId等于n，则可以查询节点n的状态，若不指定nodeId，则默认返回提供服务的节点即链接中ip所指定节点的状态。  
...
CMRestAPI使用PUT方法，推送主机信息携带参数MasterIpPort，主机信息格式为ip:port，推送备机信息携带参数StanbysInfo，备机信息格式为ip1:port1,ip2:port2,...,ipn:portn，一个应用端的demo见CMRestAPI仓库中的applicationdemo。

**其他使用说明：**  
1. 安全相关  
CMRestAPI默认使用http服务，支持配置访问白名单，可通过启动参数-w配置访问来源ip的白名单文件，白名单文件配置格式为每行一个ip地址；若要使用https服务，则可以在启动时jar包时指定系统参数server.ssl相关参数来是CMRestAPI启动https服务，或配置resource目录下的application.properties文件，配置参数示例：  
