...
密态数据库在客户端进行加密，需要在客户端进行大量的操作，包括管理数据密钥，加密敏感数据，解析并修改实际执行的SQL语句，并且识别返回到客户端的加密的数据信息。GaussDB kernel将这一系列的复杂操作，自动化的封装在前端解析中，对SQL查询中与敏感信息的加密替换，使得发送至数据库服务侧的查询任务也不会泄露用户查询意图，减少客户端的复杂安全管理及操作难度，实现用户应用开发无感知。
...
为用户提供数据加解密接口，针对用户识别的敏感信息列使用加密函数，使得数据加密后再存储在表内。
