...
**参数说明**： 指定hcom通信库目录，配合comm\_sctp\_type使用可以将Stream算子使用的libcom库替换为hcom
...
>![](public_sys-resources/icon-note.png) **说明：** 
>-   该参数需要在配置有效hcom\_link\_path的前提下生效
>-   该参数生效范围为：SPQ场景下的跨节点Stream算子
>-   可配置参数说明。
>   -   ORIGIN：不替换libcom
>   -   TCP：使用hcom通信库，并且使用TCP模式
>   -   HCCS：使用hcom通信库，并且使用HCCS模式

该参数属于POSTMASTER类型参数，请参考[表1](../DatabaseAdministrationGuide/重设参数.md#zh-cn_topic_0237121562_zh-cn_topic_0059777490_t91a6f212010f4503b24d7943aed6d846)中对应设置方法进行设置。

**取值范围**："ORIGIN", "TCP", "HCCS"

**默认值**： 'ORIGIN'

## comm\_link\_num

**参数说明**： 配置Hcom场景下节点间多连接功能
