...
- openGauss共享存储模式集群仅支持从基线版本大于等于5.0.0版本升级到之后的版本。

- openGauss共享存储模式集群不支非共享存储集群升级到共享存储集群以及共享存储集群升级到非共享存储集群，不支持RDMA模式下的升级。

- openGauss共享存储模式集群升级不再备份catalog物理文件，请在升级前使用gs_probackup备份数据，防止故障后数据丢失。
