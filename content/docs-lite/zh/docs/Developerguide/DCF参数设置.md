...
**参数说明：**是否开启DCF模式，该参数不允许修改，仅可以在安装前在xml文件中配置，如果在安装后强行修改可能会导致数据库无法启动等严重问题。
...
**参数说明：**用户安装时自定义配置信息，该参数不允许修改，仅可以在安装前在xml文件中配置，如果在安装后强行修改可能会导致数据库无法启动等严重问题。

**默认值：**字符串，安装时用户自定义配置

## dcf\_data\_path<a name="section28801541163"></a>

**参数说明：**DCF数据路径，该参数不允许修改，仅可以在安装前在xml文件中配置，如果在安装后强行修改可能会导致数据库无法启动等严重问题。

**默认值：**字符串，DN数据目录下的dcf\_data目录

## dcf\_log\_path<a name="section6335163315612"></a>

**参数说明：**DCF日志路径，该参数不允许修改，仅可以在安装前在xml文件中配置，如果在安装后强行修改可能会导致数据库无法启动等严重问题。

**默认值：**字符串，DN数据目录下的dcf\_log目录

## dcf\_node\_id<a name="section0577131618718"></a>

**参数说明：**DCF所在DN节点ID，用户安装时自定义，该参数不允许修改，仅可以在安装前在xml文件中配置，如果在安装后强行修改可能会导致数据库无法启动等严重问题。
