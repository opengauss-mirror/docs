...
1.  使用普通用户登录到openGauss轻量版包安装的主机，解压轻量版安装包到安装目录。
    ...
    ```
    tar -zxf openGauss-Lite-6.0.2-openEuler20.03-aarch64.tar.gz -C ~/openGauss
    ```
...
4.  安装执行完成后（日志中显示start datanode success），根据日志提示手动source $envfile，然后使用ps和gs\_ctl查看进程是否正常。
    ...
    ```
    omm      24209 11.9  1.0 1852000 355816 pts/0  Sl   01:54   0:33 /opt/software/openGauss/install/bin/gaussdb -D /opt/software/openGauss/data
    ...
    ```
    gs_ctl query ,datadir is /opt/software/openGauss/data
    HA state:
        local_role                     : Normal
