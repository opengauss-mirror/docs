...
<tr id="row14769111017290">
    ...
    <td class="cellrowborder" valign="top" width="75.57000000000001%" headers="mcps1.2.3.1.2 ">默认情况下openGauss只会把cursor(select xxx)和cursor(with xxx)两种场景的查询语句当成游标表达式执行，其他场景当成cursor function执行。如果开启此参数且系统中没注册cursor function，openGauss将所有的cursor()都当成表达式执行。比如cursor((select xxx))默认情况下会当成cursor function执行，打开其参数后将当成cursor表达式执行</p></td>
</tr>
<tr id="row14769111017290">
...
<tr id="row14769111017291">
    ...
    <td class="cellrowborder" valign="top" width="75.57000000000001%" headers="mcps1.2.3.1.2 ">是否以更兼容A库的方式处理数组。启用后数组比较改为无序比较、删除arr[i]后，arr[i]仍然占位，访问它会获得NULL、支持delete(x,y)函数。</p>
