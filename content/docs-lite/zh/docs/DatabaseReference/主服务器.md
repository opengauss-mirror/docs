...
-   on表示在有同步备机故障且与主机断开连接时，主机不因同步备故障而阻塞。比如有两个同步备机，一个故障，另一个正常，这个时候主机事务只会等好的这个同步备，而不被故障的同步备所阻塞；再比如走quorum协议时，一主三同步备，配置ANY 2\(node1,node2,node3\)，当node1、node3故障，node2正常时，主机业务同样不被阻塞。
