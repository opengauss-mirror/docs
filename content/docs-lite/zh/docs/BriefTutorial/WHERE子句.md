# WHERE子句<a name="ZH-CN_TOPIC_0000001210141890"></a>

当我们需要根据指定条件从表中查询数据时，就可以在SELECT语句中添加WHERE子句，从而过滤掉我们不需要数据。WHERE子句构成一个行选择表达式，用于提取那些满足指定条件的数据，如果给定的条件满足，才返回从表中提取到的具体数值。
