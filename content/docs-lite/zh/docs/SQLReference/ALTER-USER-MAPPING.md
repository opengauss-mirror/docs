...
当在OPTIONS中出现password选项时，需要保证openGauss每个节点的\\$GAUSSHOME/bin目录下存在usermapping.key.cipher和usermapping.key.rand文件，如果不存在这两个文件，请使用gs_guc工具生成并放入到每个节点的$GAUSSHOME/bin目录下。
