...
解决办法："目标表属性个数与要拷贝的源文件属性个数不一致。需要检查待copy文件的列数和表中指定的列数是否一致。"
...
解决办法：请检查文件数据格式合法性，详细信息请参见COPY章节。
...
解决办法：需要依次检查：1. 数据库状态是否正常；2. 节点之间的连接是否正常；3. 当前节点之间的网络连接是否正常；4. 所有节点上的内存资源是充足的，未发生OOM问题；
