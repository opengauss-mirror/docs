...
1.  执行如下命令下载openGauss-server代码，其中_\[git ssh address\]_表示实际代码下载地址，可在openGauss社区获取这些地址。
    ...
    >    下载地址：https://opengauss.obs.cn-south-1.myhuaweicloud.com/3.0.0/openGauss-third\_party\_binarylibs.tar.gz
...
>2.  -3rd \[binarylibs path\] 为binarylibs的路径。需要指定三方库的绝对路径。
>3.  -nopkg只编译小型化，不进行打包。编译的结果在openGauss-server/mppdb\_temp\_install目录下。 如果不指定该参数，默认会打包，打包结果在openGauss-server/output目录下。
