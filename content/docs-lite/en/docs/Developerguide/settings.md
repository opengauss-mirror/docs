...
-   **off **\(**false**,** no**,** 0**\): The primary node commits transactions without waiting for the primary node to flush logs to disks. This mode is also called asynchronous commit.
...
**Default value**: 2048 \(that is, 16 MB\)
