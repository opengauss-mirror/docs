...
-   **WITH \[ RECURSIVE \] with\_query \[, ...\]**
    ...
    Specifies one or more subqueries that can be referenced by name in the main query, which is equivalent to a temporary table that exists only in the main query and can simplify complex queries.
    ...
    ```
    -  It consists of non-recursive items, UNION items, and recursive items.
    -  Only recursive items can reference themselves.
    -  Only one recursive can reference itself in each query.

-   **plan\_hint**  clause

    Follows the  **SELECT**  keyword in the  **/\*+**<*Plan hint*\>** \*/**  format. It is used to optimize the plan of a  **SELECT**  statement block. For details, see  [Hint-based Tuning](hint-based-tuning.md). In each statement, only the first  **/\*+** *plan\*hint _**\*/**  comment block takes effect as a hint. Multiple hints can be written.
...
-   **CONNECT BY clause**
    ...
    **CONNECT BY**  indicates the recursive join condition. In the  **CONNECT BY**  condition, you can specify the  **PRIOR**  keyword for a column, indicating that the column is used as the recursive key for recursion. The  **PRIOR**  keyword can be specified only for columns in the table and cannot be specified for expressions or type conversion. If  **NOCYCLE**  is added before a recursive join condition, recursion stops when a circular record is encountered. \(Note: A SELECT statement containing the  **START WITH .. CONNECT BY**  clause does not support the FOR SHARE or UPDATE lock.\)
    ...
    1. The initial dataset is selected based on the condition in the START WITH clause. In the preceding example, ('A', 1, 0) is selected first. Then, this initial dataset is set as the working set.
    2. If the working set is not empty, the data in the working set is used as the input to query the data of the next round. The filter criteria are specified in the CONNTECT BY area. The PRIOR keyword indicates the current record. In the preceding example, **prior id = fatherid** indicates that the ID of the current record is the fatherid of the next record.
    3. Set the dataset filtered in step 2 as the working set and repeat the operation in step 2. 
    ...
    - LEVEL: indicates the node level.
    - CONNECT\_BY\_ISLEAF: indicates whether the node is a leaf node.

    In addition to pseudocolumns, the following query functions are provided:
    - **sys\_connect)by\_path(col, separtor)**: returns the connection path from the root node to the current row. The **col** parameter indicates the name of the column displayed in the path, and the **separator** parameter indicates the connector.
    - **connect_by_root(col)**: displays the top-level node of the node. **col** indicates the name of the output column.

    Currently, the default behavior of START WITH is width-first search. However, depth-first search can be implemented by working with pseudocolumns.
    ```
...
-   **GROUP BY clause**
    ...
    Condenses query results into a single row all selected rows that share the same values for the grouping expressions.
    ...
    -   ROLLUP \( \{ expression | \( expression \[, ...\] \) \} \[, ...\] \)
        ...
        **ROLLUP** is a shortcut function for generating multiple grouping sets. Different from the **CUBE** clause, **ROLLUP** does not generate all possible grouping sets based on a specific column. Instead, **ROLLUP** generates a grouping set as a subset of **ROLLUP**. **ROLLUP** assumes that there is a hierarchy between input columns, resulting in all meaningful grouping sets. For example:

        ROLLUP(c1,c2,c3) generates only four grouping sets, which is equivalent to:
        ```
        GROUPING SETS (
          (c1, c2, c3),
          (c1, c2),
        ...
        ```
    >![](public_sys-resources/icon-notice.gif) **NOTICE:** 
    > - If the **SELECT** list expression quotes some ungrouping columns and no aggregate function is used, an error is displayed unless an aggregate function is used or its function depends on grouping columns. This is because multiple values may be returned for ungrouping columns. If the grouping column is the primary key (or a subset of the primary key) of a table that contains ungrouping columns, function dependency exists.
    > - If any **GROUPING SETS**, **ROLLUP**, or **CUBE** exists as a grouping column, the **GROUP BY** clause defines several independent grouping sets. The effect is equivalent to building a **UNION ALL** between subqueries with a grouping set as their **GROUP BY** clause.
    > - Currently, **FOR UPDATE**, **FOR SHARE**, **FOR NO KEY UPDATE**, and **FOR KEY SHARE** cannot be specified together with **GROUP BY**.
...
-   **HAVING clause**
    ...
    >![](public_sys-resources/icon-notice.gif) **NOTICE:** 
    >
    >Currently, **FOR UPDATE**, **FOR SHARE**, **FOR NO KEY UPDATE**, and **FOR KEY SHARE** cannot be specified together with **HAVING**.
...
-   **WINDOW clause**
    ...
    **frame\_start**  and  **frame\_end**  can be expressed in the following forms:
    ```
    UNBOUNDED PRECEDING     // Frame starting from the first row of the partition

    value PRECEDING         // Frame at the beginning or end of the value row before the current row

    CURRENT ROW             // Frame at the beginning or end of the current row

    value FOLLOWING         // Frame at the beginning or end of the value row after the current row

    UNBOUNDED FOLLOWING     // Frame at the end of the last row of the partition
    ```
    - The default value of **frame\_start** is **UNBOUNDED PRECEDING**, and the default value of **frame\_end** is **CURRENT ROW**.
    - The value of **frame\_end** must be later than that of **frame\_start**.
    - The value cannot be a negative number or empty. It can be 0.

    >![](public_sys-resources/icon-notice.gif) **NOTICE:** 
    >For the query of column storage table, only  **row\_number**  window function is supported, and  **frame\_clause**  is not supported.

    The WINDOW clause specifies the behavior of window functions. When processing window functions with the same definition, use the WINDOW clause and reference it in OVER to simplify SQL statements. For example:
    ```
...
-   **Locking clause**
    ...
    **FOR KEY SHARE**  is similar to  **FOR SHARE**  except that its lock is weak.  **SELECT FOR UPDATE**  is blocked but  **SELECT FOR NO KEY UPDATE**  is not blocked. A key-shared lock blocks other transactions from executing  **DELETE**  or  **UPDATE**  that modifies the key value, but does not block  **UPDATE**,  **SELECT FOR NO KEY UPDATE**,  **SELECT FOR SHARE**, or  **SELECT FOR KEY SHARE**.
    ...
    To prevent the operation from waiting for the commit of other transactions, you can use  **NOWAIT**. If the selected row cannot be locked immediately, an error is reported immediately and there is no waiting. If you use  **WAIT N**  and the selected row cannot be locked immediately, the operation needs to wait for  *N_  seconds \(the value of  _N*  is of the int type with a range of 0 ≤  *N_  ≤ 2147483\). If the lock is obtained within  _N*  seconds, the operation is performed normally. Otherwise, an error is reported.
...
-- Create a multi-level menu table.
...
-- Use WITH RECURSIVE to recursively query menu relationships.
