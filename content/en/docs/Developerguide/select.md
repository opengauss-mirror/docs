...
-   **plan\_hint**  clause
    ...
    Follows the  **SELECT**  keyword in the  **/\*+**<*Plan hint*\>** \*/**  format. It is used to optimize the plan of a  **SELECT**  statement block. For details, see  [Hint-based Tuning](en-us_topic_0289900289.md). In each statement, only the first  **/\*+** *plan\*hint _**\*/**  comment block takes effect as a hint. Multiple hints can be written.
...
- **CONNECT BY clause**
  ...
  **CONNECT BY** indicates a recursive join condition. It is used together with **START WITH** to recursively traverse data.
  ```
