...
-   **WITH \[ RECURSIVE \] with\_query \[, ...\]**
    ...
    The detailed format of  **with\_query**  is as follows:  **with\_query\_name \[ \( column\_name \[, ...\] \) \] AS \[ \[ NOT \] MATERIALZED \] \( \{select | values | insert | update | delete\} \)**
...
-   **plan\_hint**  clause
    ...
    Follows the  **SELECT**  keyword in the  **/\*+**<_Plan hint_\>** \*/**  format. It is used to optimize the plan of a  **SELECT**  statement block. For details, see  [Hint-based Tuning](en-us_topic_0289900289.md). In each statement, only the first  **/\*+** _plan\_hint _**\*/**  comment block takes effect as a hint. Multiple hints can be written.
