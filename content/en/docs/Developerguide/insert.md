...
-   You must have the  **INSERT**  permission on a table to insert data to it.
...
-   **WITH \[ RECURSIVE \] with\_query \[, ...\]**
    ...
    The detailed format of  **with\_query**  is as follows: with\_query\_name \[ \(column\_name \[,...\]\) \] AS

    \( \{select | values | insert | update | delete\} \)

    --  **with\_query\_name**  specifies the name of the result set generated by a subquery. Such names can be used to access the result sets of
