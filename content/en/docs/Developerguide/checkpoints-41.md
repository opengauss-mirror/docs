...
**Parameter description**: Takes effect when  **enable\_xlog\_prune**  and  **synchronous\_commit**  are enabled. If the standby server is disconnected and the size of Xlogs is greater than the threshold, the logs are recycled. When all standby nodes are disconnected and no logical replication slot exists, logs are not recycled.
