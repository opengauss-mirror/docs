...
-   Comment out a configured client authentication policy and send semaphore to  **pg\_hba.conf**.
    ...
    >![](public_sys-resources/icon-note.gif) **NOTE:** 
    >If you need to comment out a configured authentication policy, use  **-h "HOSTTYPE DATABASE USERNAME IPADDR-WITH-IPMASK AUTHMEHOD"**, without specifying  **authentication-options**.
