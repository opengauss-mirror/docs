...
-   aes\_encrypt\(str, key\_str, init\_vector\)
    ...
    -- **key\_str**: key character string. If **key\_str** is set to **NULL**, the function returns **NULL**. For security purposes, you are advised to use a 128-bit, 192-bit, or 256-bit secure random number as the key string for the 128-bit, 192-bit, or 256-bit key (determined by block\_encryption\_mode. For details, see [Security Configuration](../database_reference/security-configuration.md)).
...
-   pg\_delete\_audit
    ...
    For details about how to use the function and details about function examples, see  [Maintaining Audit Logs](../database_administration_guide/maintaining_audit_logs.md).
