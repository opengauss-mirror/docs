...
- Currently, the ledger database provides two verification interfaces:  [ledger\_hist\_check\(text,...](../sql_reference/ledger-database-functions.md)  and  [ledger\_gchain\_check\(text...](../sql_reference/ledger-database-functions.md). When a common user invokes a verification interface, only the tables that the user has the permission to access can be verified.
