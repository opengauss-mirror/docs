...
一旦准备好了，一个事务就可以在稍后用[COMMIT PREPARED](commit_prepared.md)或  [ROLLBACK PREPARED](rollback_prepared.md)命令分别进行提交或者回滚。这些命令可以从任何会话中发出，而不光是最初执行事务的那个会话。
...
[COMMIT PREPARED](commit_prepared.md)，[ROLLBACK PREPARED](rollback_prepared.md)
