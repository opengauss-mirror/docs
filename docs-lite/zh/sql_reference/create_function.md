...
- **argtype**
    ...
    函数参数的类型。可以使用%TYPE或%ROWTYPE间接引用变量或表的类型，详细可参考存储过程章节[定义变量](define_variables.md)。
...
- **plpgsql\_body**
    ...
    PL/pgSQL函数体。用于PostgreSQL风格创建函数时，language指定为plpgsql的情况。详见[PL/pgSQL语言函数](pl_pgsql_linguistic_function.md)。
