...
通过START TRANSACTION启动事务。如果声明了隔离级别、读写模式，那么新事务就使用这些特性，类似执行了[SET TRANSACTION](set_transaction.md)。
...
[COMMIT | END](commit_end.md)，[ROLLBACK](ROLLBACK.md)，[SET TRANSACTION](set_transaction.md)
