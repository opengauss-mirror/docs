...
- regexp\_substr\(string text, pattern text \[, position int \[, occurrence int \[, flags text\]\]\]\)
  ...
  描述：正则表达式的抽取子串函数。与substr功能相似，正则表达式出现多个并列的括号时，也全部处理。该函数具体的匹配结果依赖于数据库Ctype字段,与本地节点字符序设置相关联,使用前请排查确定,避免影响匹配结果。
...
- regexp\_replace\(string, pattern, replacement \[,flags \]\)
    ...
    可选的flags参数包含零个或多个改变函数行为的单字母标记。i表示进行大小写无关的匹配，g表示替换每一个匹配的子字符串而不仅仅是第一个。m表示按照多行模式匹配。SQL语法兼容A和B的情况下，n选项在GUC参数behavior\_compat\_options的值包含aformat\_regexp\_match时，表示 . 能够匹配 '\\n' 字符，flags中没有指定n时，默认.不能匹配 '\\n' 字符；值不包含aformat\_regexp\_match时，. 默认能匹配'\\n'字符。n 选项的含义与 m 选项一致。
...
- 正则表达式细节：
    ...
    正则表达式函数是指使用了正则表达式的字符处理函数，包括 regexp_substr、regexp_count、regexp_instr、regexp_matches、regexp_split_to_array、regexp_split_to_table、regexp_replace 和 regexp_like等

    正则表达式函数的模式匹配规则说明：

        如果正则表达式（简写为RE，下同）可以匹配多个子串，要么是取最长的子串，要么是最短的，具体哪种取决于 RE 是贪婪的（首选最长匹配）还是非贪婪的（首选最短匹配）。贪婪属性通常在使用量词时体现，即需要由量词确定匹配的个数，比如 *、{n,m} 等。默认情况下，这些量词是贪婪的，即尽可能匹配更多的字符。如果需要非贪婪匹配，可以在量词后面加一个问号（?），比如 *?、{n,m}? 等。分支，即一个没有顶级|操作符的 RE，和它里面的第一个有贪婪属性的量化原子有着同样的贪婪性。匹配时按照分支或者整个 RE 作为一个整体匹配最长或者最短的可能子串。如下面的例子：
        ...
        ```SQL
            ...
            结果：1
        ```
        在上面的例子中，因为 Y* 是贪婪的， 因此第一个 RE 整体是贪婪的，所以匹配了最长的子串，即 Y123，输出是这里的圆括号包围的部分即123。第二个 RE 是非贪婪的， 因为 Y*？ 是非贪婪的，所以匹配了最短的子串，即 Y1，最后输出括号中1，也就是说，虽然子表达式[0-9]{1,3}是贪婪的，但是它不能修改 RE 总体匹配长度的决定； 因此它被迫只匹配1。

        简而言之，如果一个 RE 同时包含贪婪和非贪婪的子表达式，那么总的匹配长度要么是尽可能长，要么是尽可能短，这取决于给整个 RE 赋予的属性。
