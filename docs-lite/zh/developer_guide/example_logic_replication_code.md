...
针对逻辑复制的配置选项，除了在[逻辑解码](../database_om_guide/logical_decoding_overview.md)中的配置选项外，还有专门给JDBC等流式解码工具增加的配置项，如下所示：
...
2. 解码格式
    ...
    >      3. 【该部分为可选项】接下来1字节字母如果为X，则代表后面的8字节uint64表示xid。  
    >      4. 【该部分为可选项】接下来的1字节字母如果为T，则代表后面4字节uint32表示该事务commit时间戳长度，再后面等同于该长度的字符为时间戳字符串。
