...
<tr id="row14769111017290">
    ...
    <td class="cellrowborder" valign="top" width="75.57000000000001%" headers="mcps1.2.3.1.2 ">默认情况下openGuass只会把cursor(select xxx)和cursor(with xxx)两种场景的查询语句当成游标表达式执行，其他场景当成cursor function执行。如果开启此参数且系统中没注册cursor function，openGuass将所有的cursor()都当成表达式执行。比如cursor((select xxx))默认情况下会当成cursor function执行，打开其参数后将当成cursor表达式执行</td>
</tr>
<tr id="row14769111017290">
