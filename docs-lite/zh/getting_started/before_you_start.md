# 使用openGauss<a name="ZH-CN_TOPIC_0289900873"></a>
...
2. 连接数据库。
    ...
    其中，postgres为openGauss安装完成后默认生成的数据库。初始可以连接到此数据库进行新数据库的创建。8000为数据库主节点的端口号，需根据openGauss的实际情况做替换，请参考[确认连接信息](confirming_connection_information.md)获取。

    **引申信息：**

    - 使用数据库前，需先使用客户端程序或工具连接到数据库，然后就可以通过客户端程序或工具执行SQL来使用数据库了。
    - gsql是本产品提供的命令行方式的数据库连接工具。更多的数据库连接办法可参考[连接数据库](gsql_connection_and_usage.md)。
...
3. 创建数据库用户。
    ...
    使用GRANT命令进行相关权限设置，具体操作请参考[GRANT](../sql_reference/GRANT.md)。
...
4. 创建数据库。
    ...
    **引申信息：**
    ...
    其中hr\_local为表空间名称，关于如何创建表空间，请参考[创建和管理表空间](../database_administration_guide/creating_and_managing_tablespaces.md)。
