...
- 若服务器已安装KAE，并配置相应路径到LD_LIBRARY_PATH，则可使用硬件加速的KAEZstd与KAEZlib代替原生ZSTD和ZLIB，使相应压缩和解压接口的效率提升，CPU使用率降低。KAE简介与安装指导见：[KAE产品简介](https://www.hikunpeng.com/document/detail/zh/kunpengsdss/basicAccelFeatures/kaeebspdk/kunpengspdk_16_0011.html)。
...
段页式的详细介绍可见[段页式存储结构](../database_administration_guide/segmentation_with_paging_storage_structure.md)。段页式下，数据分布满足如下特点：
...
相关参数见[高效数据压缩算法相关参数](../database_reference/parameters_related_to_efficient_data_compression_algorithms.md)。
