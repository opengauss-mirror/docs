...
- 当在异常情况或表被损坏时需要使用[ledger\_gchain\_repair\(tex...](../sql_reference/ledger_database_functions.md)或[ledger\_hist\_repair\(text,...](../sql_reference/ledger_database_functions.md)接口对全局区块表或用户历史表进行修复，修复后调用全局区块表或用户历史表校验接口结果为true。
...
- 修复全局区块表的接口为pg\_catalog.ledger\_gchain\_repair，操作为：
