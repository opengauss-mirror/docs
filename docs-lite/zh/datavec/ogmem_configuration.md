...
完整字段参考：[config/ogmem.reference.yaml](https://gitcode.com/opengauss/oG-Memory/blob/dev/config/ogmem.reference.yaml) 和 [OGMEMORY_ENV.md](https://gitcode.com/opengauss/oG-Memory/blob/dev/docs/OGMEMORY_ENV.md)。
...
| 安装方式 | 适合场景 | 交互式生成文件 |
| --- | --- | --- |
| Headless 本地服务 | 只启动 oG-Memory HTTP 服务，供 SDK、脚本或已有系统调用 | `config/ogmem.yaml` |
| Agent Plugin | 接入本机已有 OpenClaw 或 Claude Code | `config/ogmem.yaml`，以及 `openclaw.plugin.json` 或 `.claude/settings.json` |
| Docker 一体化部署 | 一次部署 OpenClaw Gateway、oG-Memory、可选 openGauss | `deploy/deploy.env`、`deploy/ogmemory.yaml` |
...
service:
  http_port: 8090              # oG-Memory HTTP 服务端口
...
选择 `Agent Plugin (OpenClaw / Claude Code)` 后，向导会先生成 oG-Memory 自身配置：
...
核心作用是把 OpenClaw Gateway 的模型配置和 oG-Memory 地址连起来。实际文件会包含 OpenClaw 所需的完整 schema，可以重点检查这些字段：
...
注意：`plugin` 模式不会替你安装 OpenClaw 或 Claude Code 本体。它只生成 oG-Memory 配置和 Agent 侧接入配置；OpenClaw / Claude Code 需要用户本机已经可用。
...
`deploy/ogmemory.yaml` 是容器内 oG-Memory 使用的配置，里面会引用 `deploy.env`：
...
service:
  http_port: 8090              # 容器内 oG-Memory HTTP 服务端口
  workers: 2                   # HTTP 服务 worker 数
...
一般优先改 YAML；环境变量适合临时覆盖、容器部署或 CI。完整列表见 [OGMEMORY_ENV.md](https://gitcode.com/opengauss/oG-Memory/blob/dev/docs/OGMEMORY_ENV.md)。
...
| 文档 | 说明 |
| --- | --- |
| [config/ogmem.reference.yaml](https://gitcode.com/opengauss/oG-Memory/blob/dev/config/ogmem.reference.yaml) | 完整 YAML 模板 |
| [OGMEMORY_ENV.md](https://gitcode.com/opengauss/oG-Memory/blob/dev/docs/OGMEMORY_ENV.md) | 完整环境变量说明 |
| [deploy/README.md](https://gitcode.com/opengauss/oG-Memory/blob/dev/deploy/README.md) | Docker 一体化部署 |
| [openclaw_context_engine_plugin/ENV.md](https://gitcode.com/opengauss/oG-Memory/blob/dev/openclaw_context_engine_plugin/ENV.md) | OpenClaw 插件变量 |
